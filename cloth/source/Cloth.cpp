...
void Cloth::renderGeometry()
{
	...
	//---draw
	glDrawArrays(GL_LINE_STRIP, 0, triangle_vec.size());  //uncomment to see the mesh drawn as triangle strips
...
void Cloth::MakeVertices(int width, int height)
{
	...
	for (int i = 0; i < height; i++)
	{
		for (int j = 0; j < width; j++)
			vertices.push_back(Vector(float(i), float(j), 0.0 ));
	}
	...
	//	vtexcoord.push_back(Vector(*itr).x() / float(width), (*itr).z() / float(height)));
